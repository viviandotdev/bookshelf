...
export function getUserBookInfo(
    ...
    if (objectFromCSV['Bookshelves']) {
        const cleanShelves = objectFromCSV['Bookshelves']
            .split(',')
            .map((shelf) => shelf.trim());
        const excludedShelves = ['to-read', 'currently-reading', 'read', 'up-next', 'did-not-finish'];
        shelves = cleanShelves.filter((shelf) => !excludedShelves.includes(shelf));
    ...
    if (objectFromCSV['Exclusive Shelf']) {
        ...
        } else if (objectFromCSV['Exclusive Shelf'] == 'up-next') {
            status = READING_STATUS.UP_NEXT;
        } else if (objectFromCSV['Exclusive Shelf'] == 'did-not-finish') {
            status = READING_STATUS.DID_NOT_FINISH;
        }
    }
