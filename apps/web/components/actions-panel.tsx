...
function ActionGroup() {
  return (
    <div className="grid grid-cols-3 gap-4 bg-secondary rounded-lg p-3">
      <ActionItem icon={<Icons.book className="h-8 w-8 " />} label="Read" />
      <ActionItem
  ...
  return (
    <>
      <div className="rounded-lg flex flex-col gap-1 items-center text-sm text-muted-foreground font-light">
        ...
        <span>Rating</span>
          <Rating halfFillMode="box" itemStyles={myStyles} style={{ maxWidth: 200 }} value={rating} onChange={setRating} />
        </div>
        <div className="bg-secondary items-center text-center w-[fill-available] rounded-lg p-2 cursor-pointer">Summarize</div>
        <div className="bg-secondary items-center text-center w-[fill-available] rounded-lg p-2 cursor-pointer">Add to lists</div>
      </div>
    </>
  );
}
