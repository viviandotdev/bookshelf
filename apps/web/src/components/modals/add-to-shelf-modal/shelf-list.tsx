...
export const ShelfList = ({ focus, options, control }: any) => {
  ...
  const [selectedShelves, setSelectedShelves] = React.useState(
    list.value || []
  );
            ...
            return (
              <div key={option.name}>
                <Button
                  ...
                  className='ml-0 flex w-full justify-start rounded-none bg-white pl-2 text-beige hover:bg-beige-100'
                  key={index}
                >
                  <Icons.shelf className='mr-4 h-5 w-5' />
                  {option.name}
                </Button>
              </div>
            );
