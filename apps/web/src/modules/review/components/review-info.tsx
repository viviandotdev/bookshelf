...
export const ReviewInfo: React.FC<ReviewInfoProps> = ({ review }) => {
  ...
  return (
    <div>
      <div className=' text-base font-semibold'>
        <span className='flex items-center'>
          <UserAvatar
            ...
            className='h-8 w-8'
          />
          <span className='mx-2 ml-4'>Reviewed by</span>
          <span className='font-bold  '>
            {' '}
            {review.userBook?.user?.username}
          </span>
          {rating > 0 && <Dot />}
          {Rating(rating)}
        </span>
      ...
      <div className='mt-2 max-h-64 gap-2 overflow-hidden text-sm'>
        <div className='text-base font-light'>
          Reviewed on {formatDate(review.createdAt)}
        </div>
        <div className={`mt-2 leading-normal`}>{review.content}</div>
      </div>
    </div>
  );
};
