    ...
    return (
        <>
            {shelf.name !== 'Favorites' && shelf.name !== 'Owned' && (
                <div
                    className={`${selectedShelfSlug && shelf.slug === selectedShelfSlug || selectedShelfSlug && shelf.name === selectedShelfSlug
                        ? 'bg-beige-100 text-beige-700'
                        : 'text-gray-400'
                        } mr-4 flex justify-between rounded-lg px-3 text-base font-normal ${isHovered ? 'hover:text-beige-700' : ''
                        }`}
                ...
                >
                    ...
                    <span
                        className={`flex cursor-pointer items-center rounded-sm px-1 text-sm ${isHovered ? 'text-beige-700' : 'text-gray-400'
                            }`}
                    >
                        {isShelves ? (
